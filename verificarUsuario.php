...
if ($filasAdmin>0) {
  $_SESSION["usuario"]=$usuario;
    header("location: php/adminInicio.php?nombre=$usuario");
}
else {
   if ($filasProfesor>0) {
    $_SESSION["usuario"]=$usuario;
    header("location: php/profesorInicio.php?nombre=$usuario");
    }else {
       if ($filasEstudiante>0) {
        $_SESSION["usuario"]=$usuario;
        header("location: php/estudianteInicio.php?nombre=$usuario");
       }else {
